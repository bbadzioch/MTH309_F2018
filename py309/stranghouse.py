...
'''
Usage:
python stranghouse.py
python stranghouse.py symmetric
python stranghouse.py diagonal
python stranghouse.py orthogonal
...
SYMMETRIC  = len(sys.argv)>1 and sys.argv[1]=='symmetric'
DIAGONAL   = len(sys.argv)>1 and sys.argv[1]=='diagonal'
ORTHOGONAL = len(sys.argv)>1 and sys.argv[1]=='orthogonal'
...
def move(event):
    global buttonisdown,xyinv,dragging,A1,A2,grid_digits
    if buttonisdown:
...
def press(event):
    global xyinv,A1,A2,buttonisdown,dragging,grid_digits
